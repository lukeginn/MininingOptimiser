...
def merging_clusters(feed_blend_clusters, controllables_clusters, feed_blend_training_features, controllables_training_features, path = None):

    feed_blend_training_features = [feature + '_historical_actuals' for feature in feed_blend_training_features]
    controllables_training_features = [feature + '_historical_actuals' for feature in controllables_training_features]
    ...
    controllables_clusters_filtered = controllables_clusters[
        [feature for feature in controllables_training_features
         if feature not in feed_blend_training_features]
    ]

    combinations = list(itertools.product(feed_blend_clusters_filtered.iterrows(), controllables_clusters_filtered.iterrows()))
    merged_clusters = pd.DataFrame([
        {'feed_blend_cluster_id': feed_blend[0], 'controllables_cluster_id': controllables[0], **feed_blend[1], **controllables[1]}
        for feed_blend, controllables in combinations
    ])

    # Reorder columns
    merged_clusters = merged_clusters[['feed_blend_cluster_id', 'controllables_cluster_id'] + feed_blend_training_features + controllables_training_features + feed_blend_clusters_columns_not_in_combined]

    if path:
        export_merged_clusters(
