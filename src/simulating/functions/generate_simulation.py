...
def generate_simulations(
    ...
    simulation_results, shapley_results = (
        generating_simulation_predictions_for_all_simulation_data(
    ...
    if informational_features is not None:
        informational_features = [
            feature + "_historical_actuals" for feature in informational_features
        ]
        informational_features = [
            feature
        ...
        simulation_results = pd.concat(
            [simulation_results, cluster_centers[informational_features]], axis=1
        )
    ...
    if feed_blend_and_controllables_modelling:
        ...
        controllables_features = [
            feature + "_historical_actuals" for feature in controllables_features
        ]
...
def generating_simulation_predictions_for_all_simulation_data(
    simulation_data,
    model_choice,
    best_models,
    training_features,
    ...
    for index, one_simulation in simulation_data.iterrows():
        ...
        simulation_predictions, simulation_shapley_values = (
            generating_simulation_predictions(
...
def plot_individual_shapley_values(shapley_results, features, path):
    for index, row in shapley_results.iterrows():
        ...
        shap.summary_plot(
            ...
            features + ["intercept", "final_prediction"],
            plot_type="bar",
            show=False,
        )
