...
def renaming_columns(data):
    ...
    data.columns = [col.replace('%', '') + '_PERC' if '%' in col else col for col in data.columns]
...
def converting_columns(data):
    ...
    data["FLOTATION_COLUMN_01_AIR_FLOW"] = pd.to_numeric(data["FLOTATION_COLUMN_01_AIR_FLOW"])
    data["FLOTATION_COLUMN_02_AIR_FLOW"] = pd.to_numeric(data["FLOTATION_COLUMN_02_AIR_FLOW"])
    data["FLOTATION_COLUMN_03_AIR_FLOW"] = pd.to_numeric(data["FLOTATION_COLUMN_03_AIR_FLOW"])
    data["FLOTATION_COLUMN_04_AIR_FLOW"] = pd.to_numeric(data["FLOTATION_COLUMN_04_AIR_FLOW"])
    data["FLOTATION_COLUMN_05_AIR_FLOW"] = pd.to_numeric(data["FLOTATION_COLUMN_05_AIR_FLOW"])
    data["FLOTATION_COLUMN_06_AIR_FLOW"] = pd.to_numeric(data["FLOTATION_COLUMN_06_AIR_FLOW"])
    data["FLOTATION_COLUMN_07_AIR_FLOW"] = pd.to_numeric(data["FLOTATION_COLUMN_07_AIR_FLOW"])
