...
class CorrelationMatrixProcessor:
    ...
    def run(self) -> pd.DataFrame:
        ...
        correlation_matrix = self._create_correlation_matrix(
            data=data, method=self.method
        )

        if self.csv_path is not None:
            self._saving_correlation_matrix(
                correlation_matrix=correlation_matrix, path=self.csv_path
            )

        if self.plotting_path is not None:
            self._plot_correlation_matrix(
                correlation_matrix=correlation_matrix, path=self.plotting_path
            )
    ...
    def _saving_correlation_matrix(
        self, correlation_matrix: pd.DataFrame, path: str
    ) -> None:
        logger.info(f"Saving correlation matrix to {path}")
        correlation_matrix.to_csv(path)

    def _plot_correlation_matrix(
        self, correlation_matrix: pd.DataFrame, path: Optional[str] = None
    ) -> None:
