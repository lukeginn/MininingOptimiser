...
class FeatureSelectorProcessor :
    ...
    def run(self) -> Dict[str, Any]:
        ...
        if self.filter_low_variance:
            ...
            self.training_features = self._filter_out_zero_or_near_zero_variance_features()
            training_features_per_method["low_variance"] = self._get_alphabetical_features()

        if self.filter_univariable_feature_importance:
            logger.info("Filtering features via univariable feature importance")
            self.training_features, univariable_feature_importance = self._select_features_via_univariable_feature_importance()
            training_features_per_method["univariable_feature_importance"] = self._get_alphabetical_features()
        else:
        ...
        if self.filter_feature_importance:
            ...
            self.training_features, feature_importance = self._select_features_via_feature_importance()
            training_features_per_method["feature_importance"] = self._get_alphabetical_features()
        else:
    ...
    def _filter_out_zero_or_near_zero_variance_features(self) -> List[str]:
        ...
        if self.data[self.training_features].var().max() < self.low_variance_threshold:
            logger.warning("No feature in X meets the variance threshold. Returning original features.")
            return self.training_features
        else:
            variance_selector.fit(self.data[self.training_features])
            return self.data[self.training_features].columns[variance_selector.get_support()].tolist()
    ...
    def _select_features_via_feature_importance(self) -> (List[str], pd.DataFrame):
        ...
        feature_selected_features = self._apply_filter_to_feature_importance(feature_importance)
        return feature_selected_features, feature_importance

    def _apply_filter_to_feature_importance(self, feature_importance: pd.DataFrame) -> List[str]:
        feature_importance = feature_importance[feature_importance["IMPORTANCE"] > self.feature_importance_threshold]
        return feature_importance["FEATURES"].tolist()

    def _select_features_via_univariable_feature_importance(self) -> (List[str], pd.DataFrame):
        ...
        # Apply the threshold
        univariable_feature_importance = univariable_feature_importance[
            abs(univariable_feature_importance["IMPORTANCE"]) > self.univariable_feature_importance_threshold
        ]
